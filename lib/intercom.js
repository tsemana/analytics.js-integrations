...
Intercom.prototype.identify = function (identify) {
  ...
  if (cachedGroup && cachedGroup.traits()) {
    traits.company = traits.company || {};
    var cachedTraits = cachedGroup.traits();
    for (var prop in cachedTraits) {
      if (!cachedTraits.hasOwnProperty(prop)) continue;
      if (traits.company[prop] == null) {
        traits.company[prop] = cachedTraits[prop];
      }
    }
  }

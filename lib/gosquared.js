...
GoSquared.prototype.identify = function (identify) {
  ...
  var uid = identify.uid();
  var id = identify.userId();

  if (uid) traits.userID = uid; // gosquared recognizes this in `Visitor`
  if (uid) window.GoSquared.UserName = uid;
  if (email || username || uid) {
    window.GoSquared.VisitorName = email
      || username
      || id;
  }
